            ...
            for (const batch of batches) {
                ...
                await Promise.allSettled(batch.map(async (node) => {
                    try {
                        ...
                        if (latency !== -1) {
                            ...
                            if (node.auth_user && node.auth_password) {
                                try {
                                    ...
                                    logger.warn(`MikroTik API failed for ${node.name} (${node.ip_address}): ${err.message}`);
                        ...
                        logger.error(`Critical error processing node ${node.name} (${node.id})`, { error: nodeErr.message });
                    }
                }));
            }
